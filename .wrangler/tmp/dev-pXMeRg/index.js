...
function generateSubstitution(userId) {
...
__name(generateSubstitution, "generateSubstitution");
function generateScytale(userId) {
...
__name(generateScytale, "generateScytale");
function generateBookCipher(userId) {
...
__name(generateBookCipher, "generateBookCipher");
function generateMapColoring(userId) {
  const nodes = [
    { id: "A", x: 20, y: 20 },
    { id: "B", x: 80, y: 20 },
    { id: "C", x: 50, y: 50 },
...
__name(generateMapColoring, "generateMapColoring");
async function generateRandomPuzzle(userId, geminiKey) {
  if (geminiKey) {
    try {
...
__name(generateRandomPuzzle, "generateRandomPuzzle");
...
app.post("/api/auth/request", async (c) => {
  ...
  await c.env.DB.prepare("INSERT INTO auth_tokens (token, email, expires_at) VALUES (?, ?, ?)").bind(token, email, expiresAt).run();
...
app.post("/api/auth/verify", async (c) => {
  ...
  if (!user) {
    ...
    const username = `Agent_${Math.floor(Math.random() * 1e4)}`;
  ...
  await c.env.DB.prepare("INSERT INTO sessions (session_id, user_id, expires_at) VALUES (?, ?, ?)").bind(sessionId, user.id, sessionExpiresAt).run();
...
app.get("/api/puzzles", authenticate, async (c) => {
  ...
  while (unsolved.length < 4) {
    ...
    await c.env.DB.prepare(
      ...
      newPuzzle.type,
      newPuzzle.title,
      newPuzzle.description,
      newPuzzle.difficulty,
      newPuzzle.multiplier,
      newPuzzle.puzzle_data,
      newPuzzle.answer
    ).run();
  ...
  const formatPuzzle = /* @__PURE__ */ __name((p) => ({
    id: p.id,
    type: p.type,
    title: p.title,
...
app.post("/api/challenges", authenticate, async (c) => {
  ...
  if (target.id === user.id) return c.json({ error: "Cannot challenge yourself" }, 400);
  const today = (/* @__PURE__ */ new Date()).toISOString().split("T")[0];
  if (!user.is_pro) {
    const aiUsage = await c.env.DB.prepare(
      "SELECT count FROM daily_ai_usage WHERE user_id = ? AND date = ?"
    ).bind(user.id, today).first();
    const dailyCount = aiUsage?.count ?? 0;
    if (dailyCount >= 3) {
  ...
  ).bind(challengeId, user.id, target.id, puzzle.id, JSON.stringify({
    type: puzzle.type,
    title: puzzle.title,
...
app.get("/api/challenges", authenticate, async (c) => {
  ...
  const withResults = /* @__PURE__ */ __name(async (list) => {
...
app.post("/api/challenges/:id/solve", authenticate, async (c) => {
  ...
  if (isCorrect) {
    ...
    const count3 = resultCount[0]?.cnt ?? 0;
    const newStatus = count3 >= 2 ? "completed" : "active";
  ...
  if (puzzle.type === "map_coloring") {
    try {
      ...
      for (const [u, v] of data.edges) {
        if (coloring[u] === void 0 || coloring[v] === void 0) return false;
...
// .wrangler/tmp/bundle-0FU3Ao/middleware-insertion-facade.js
...
// .wrangler/tmp/bundle-0FU3Ao/middleware-loader.entry.ts
